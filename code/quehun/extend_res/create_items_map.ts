...
function replacePathSign(str: string) {
    if (!str) return str;
...
fs.writeFileSync(
    ...
    JSON.stringify(old_to_new, null, 4)
);
fs.writeFileSync(
    "code/quehun/extend_res/output/new_to_old.json",
    JSON.stringify(new_to_old, null, 4)
);
fs.writeFileSync(
    "code/quehun/extend_res/output/unsed.json",
    JSON.stringify(Object.keys(itemsMap.unusedItems), null, 4)
);
// fs.writeFileSync(
//     "code/quehun/extend_res/output/activity_items.json",
//     JSON.stringify(itemsMap.activityItems, null, 4)
// );
